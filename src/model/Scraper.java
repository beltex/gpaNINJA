/*
 * York site scraper for logging in to get transcript and program
 *
 */
...
public class Scraper {
    ...
    public Scraper() {
        ...
        // Hard-coded values in the York login page HTML that are passed as part of the post request
     ...
     *
     * @param username Student Passport York ID
     * @param password Max 8 characters
     * @return An ArrayList of Documents, the transcript and major HTML pages
    ...
    public ArrayList<Document> getDocs(String username, String password) throws HttpStatusException {
        ...
        try {
            ...
            e.printStackTrace();
            throw new HttpStatusException("E1 - URL_TRANS - INITIAL - HTTP Request Failure", res.statusCode(), URL_TRANS);
        }
        ...
        // On failed login (bad creds), socket timeout occurs,
        // thus to prevent this, we increase timeout. Could use
        // this to detect bad creds, but that would be a
        // bit of a hack :)
        ...
        try {
            ...
            e.printStackTrace();
            throw new HttpStatusException("E1 - URL_LOGIN - LOGIN - HTTP Request Failure", res.statusCode(), URL_LOGIN);
        }

        // On failed login (bad creds), York site still
        // returns 200 response, thus we have to check for cookies
        if (!res.cookies().containsKey("mayaauth") &&
            !res.cookies().containsKey("pyauth")) {
            // Login failed
            throw new HttpStatusException("E2 - URL_LOGIN - LOGIN - Bad Creds", res.statusCode(), URL_LOGIN);
        }
        ...
        try {
            ...
            e.printStackTrace();
            throw new HttpStatusException("E1 - URL_TRANS - SECOND - HTTP Request Failure", res.statusCode(), URL_TRANS);
        }
        ...
        try {
            ...
            e.printStackTrace();
            throw new HttpStatusException("E1 - URL_PROGRAM - HTTP Request Failure", res.statusCode(), URL_PROGRAM);
        }


        // 5th Request
        // Now logout to invalidate the session cookies for safety
        conn.url(URL_LOGOUT);
        try {
            res = conn.method(Method.POST).execute();
        } catch (Exception e) {
            e.printStackTrace();
            throw new HttpStatusException("E1 - URL_LOGIN - LOGOUT - HTTP Request Failure", res.statusCode(), URL_LOGIN);
        }
